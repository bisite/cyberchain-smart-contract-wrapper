...
import { FeeMarketEIP1559TxData, FeeMarketEIP1559Transaction, LegacyTxData, LegacyTransaction } from '@cyberchain/tx';
import { Common } from '@ethereumjs/common';
import { Web3RPCClient } from './rpc-client';
import { bigintsToHex, parseAddress, parseQuantity, toHex } from './utils';
import { privateKeyToMixedAddress } from './account';
...
async function sendFeeMarketTransaction(to: AddressLike, data: BytesLike, value: QuantityLike, options: TransactionSendingOptions): Promise<TransactionReceipt> {
    ...
    const accountAddress = privateKeyToMixedAddress(options.signKey, parseAddress(options.complementaryAddress));
    const complementaryAddress = EthereumJsUtil.Address.fromString(parseAddress(options.complementaryAddress));
    ...
    tx = tx.sign(options.signKey, complementaryAddress);
